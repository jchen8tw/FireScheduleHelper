...
export default function App() {
  ...
  const handleReadSchedule = () => {
    if (
      confirm(
        "確定要讀取最新勤務表嗎？這將會更新上班人員名單，並【清空】現有的作戰編組！",
      )
    ) {
      chrome.storage?.local?.get(
        ["pendingIdToNameMap", "pendingOnDutyIds"],
        (data) => {
          if (
            !data.pendingIdToNameMap ||
            Object.keys(data.pendingIdToNameMap).length === 0
          ) {
  ...
  const ROLE_LABELS: Record<string, string> = {
    fire_user_id_a: "火警值班",
    attack_driver: "攻擊車 司機",
    attack_leader: "攻擊車 帶隊官",
    attack_nozzle: "攻擊車 瞄子手",
    attack_asst_nozzle: "攻擊車 副瞄子手",
    attack_search: "攻擊車 破壞搜救手",
    relay_driver: "中繼車 司機",
    relay_nozzle: "中繼車 瞄子手",
    ladder_driver: "雲梯車 司機",
    ambulance_driver: "救護車 司機",
    ambulance_emt: "救護車 救護技術員",
  };
  ...
  const renderSummary = () => {
    ...
    for (const [title, roles] of Object.entries(groups)) {
      const items = roles
        .filter((r) => combatGroup[r])
        .map((r) => (
          <span key={r}>
            <span className="text-xs text-muted-foreground">
              {ROLE_LABELS[r]?.split(" ").pop() || r}:
            </span>{" "}
            <span className="font-bold">{combatGroup[r].name}</span>
          </span>
        ));
      if (items.length > 0) {
        elements.push(
          <div key={title} className="mb-3">
            <h4 className="text-[13px] font-semibold text-muted-foreground mb-1">
              {title}
            </h4>
        ...
        <TabsContent value="CombatGroup" className="flex flex-col gap-4">
          <Card className="shadow-sm">
            <CardContent className="p-4 flex flex-col gap-4">
              ...
              <div className="min-h-[100px]">{renderSummary()}</div>
            </CardContent>
          </Card>
        </TabsContent>
            ...
            <CardContent className="p-4 flex flex-col gap-4">
              <div className="flex items-center space-x-2">
                <Switch
                  ...
                  onCheckedChange={saveOptions}
                />
                <Label
                  htmlFor="enableReplace"
              ...
              <div className="flex items-center space-x-2">
                <Switch
                  id="enableRemoveDutyAlert"
                  checked={enableRemoveDutyAlert}
                  onCheckedChange={saveRemoveDutyAlert}
                />
                <Label
                  htmlFor="enableRemoveDutyAlert"
                  className="text-sm font-medium leading-none"
                >
                  去除新值班系統彈窗警示功能
                </Label>
